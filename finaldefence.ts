...
const doubleMap = document.querySelector('#mapDouble');
const symmetryMap = document.querySelector('#mapSymmetry');
const castleMap = document.querySelector('#mapBrokenCastle');
const stoneMap = document.querySelector('#mapStoneCliffs');
...
const restartButton2 = document.querySelector('#startOver2');
...
class Enemy {
    ...
    constructor(x :number, y :number, health :number, speed :number, direction :string, radius :number, color :string, enemymoney, armor :number, shield :number, regen :number, path :number){
        this.x = x;
        this.y = y;
        this.radius = radius;
        this.color = color;
...
class Projectile {
    ...
    pierce: number; //enemies it can hit
    enemiesHit :Enemy[];

    constructor(x :number, y :number, damage :number, speed :number, size :number, color :string, target :Enemy, maxlifespan, ap :number, pierce :number){
        this.ap = ap;
        this.x = x;
        this.y = y;
        this.damage = damage;
...
//creates the wave
function spawnWave(numenemies :number, density :number, health :number, speed :number, size :number, color :string, Emoney :number, armor :number, shield :number, regen :number, ePath = 0, boss = new Enemy(0, 0, 0, 0, "E", 0, "black", 0, 0, 0, 0, 1)) :void {
...
function targetinellipse(towerX: number, towerY:number, towerRange:number, targetX: number, targetY :number, target :Enemy){
    ...
    enemies.forEach(function (enemy, index) {
        ...
        shots.forEach(function(shot, index){
            if(shot.x > enemy.x-enemy.radius-shot.size && shot.x < enemy.x+enemy.radius+shot.size && shot.y > enemy.y-enemy.radius-shot.size && shot.y < enemy.y+enemy.radius+shot.size){
                ...
                var alreadyhit = 0;
                shot.enemiesHit.forEach(enemyhit => {
                    if(enemyhit == enemy){
                        alreadyhit = 1;
                    }
                });
...
function canupgrade(max :number){
    for(var i=0; i<towers.length; i++){
        if(towers[i].selected == 1){
            var towernum = -1;
...
function findClosestTowers(Tower, num){//tower is the tower we are trying to find the closest of, num is how many.
    const distances = [];
    for (let i = 0; i < towers.length; i++) {
        if(towers[i].type != "buffer" && towers[i].buffs == 0){
            const newdistance = Math.sqrt(Math.pow(towers[i].x - Tower.x, 2) + Math.pow(towers[i].y - Tower.y, 2));
            //@ts-ignore
    ...
    //@ts-ignore
    return distances.slice(0, num).map(obj => obj.tower);
}

function lineCrossesCircle(m, slope, h, k, r) { //function created by CHATGPT
